...
if 'function scheduleFloatingZoom()' not in s:
 for p in json.loads(Path('patch/zoom-flutuante.json').read_text(encoding='utf-8')):
...
if '/zoom-flutuante.css' not in h:h=h.replace('</head>','    <link rel="stylesheet" href="/zoom-flutuante.css" />\n  </head>',1)
def bump(m):
 name=m.group(3)
 if not name.endswith('-zoom-flutuante-v2'):name+='-zoom-flutuante-v2'
...
if "'/zoom-flutuante.css'" not in w:
 w,n=re.subn(r'(const SHELL\s*=\s*\[)',r"\1'/zoom-flutuante.css', ",w,count=1)
...
print('Zoom flutuante aplicado: menos, restaurar 100%, mais. Visível também em 100%.')
